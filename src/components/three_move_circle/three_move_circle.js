...
import cssStyle from "./three_move_circle.module.css";
import { Motion, spring } from "react-motion";
import {getData} from "../../common/getDataUtil";
import {initThreeDom} from "../../common/threeUtil";
import {interactData} from "../../common/util";

export default class ThreeMoveCircle extends React.Component {
    constructor(props) {
        super(props);
        ...
        this.geometryList = [];
        this.materialList = [];
        this.circleGeometryList = [];
        this.circleMaterialList = [];
        this.meshList = [];
        this.clickObjects = [];
        this.keyParams = {};
    }
    ...
    componentDidMount() {
        setTimeout(()=>{
            ...
            this.p = new Promise((resolve) => {
                ...
                loader.load( './font/FZLanTingHei_taizhou_Regular.json',  ( font ) => {
                    this.font = font;

                    loader.load( './font/QuartzEF_Regular.json',  ( font ) => {
                        this.numFont = font;
    ...
    destroyOld(){
        let group = [];
        this.groupOther.children.forEach((mesh)=>{
            group.push(mesh);
        });
        group.forEach((mesh)=>{
            this.groupOther.remove(mesh);
        });
        group = [];
        this.groupCircle.children.forEach((mesh)=>{
            group.push(mesh);
        });
        group.forEach((mesh)=>{
            this.groupCircle.remove(mesh);
        });
        group = [];
        this.circleGeometryList.forEach((item)=>{
            item.dispose();
        });
        this.circleMaterialList.forEach((item)=>{
            item.dispose();
        });
        this.circleGeometryList = [];
        this.circleMaterialList = [];
    ...
    initThree(){
        ...
        this.groupOther = new THREE.Group();
        this.scene.add(this.groupOther);
    ...
    addMoveCircle(result){
        ...
        if(this.font){
            ...
            const shapes = this.font.generateShapes( result[0].name.split("").join(" "), 1.5, 2 );
            ...
            textShape.rotateY(Math.PI/4);
            const text = new THREE.Mesh( textShape, matText );
            text.position.set(0,-2,0);
            this.groupOther.add(text);
            this.circleGeometryList.push(textShape);
            this.circleMaterialList.push(matText);
        }
        //数字
        if(this.numFont){
            const textShape = new THREE.BufferGeometry();
            const shapes = this.numFont.generateShapes( result[0].num.toString(), 1.5, 2 );
            const geometryText = new THREE.ShapeGeometry( shapes );
            geometryText.computeBoundingBox();
            const xMid = - 0.5 * ( geometryText.boundingBox.max.x - geometryText.boundingBox.min.x );
            geometryText.translate( xMid, 0, 0 );
        ...
        for(let i = 1;i < result.length;i ++){
            // setTimeout(()=>{
            // },i*1460+100);
            this.addOnePart(i,times,geometry.attributes.position.array,result);
        }
        this.showMoveCircle = true;
    }

    addOnePart(index,timeArr,posArr,result){
        //图标材质
        ...
        circle.name = 'clickItemCircleMove_'+index;
        thisGroup.add(circle);
        this.clickObjects.push(circle);
        ...
        circleMesh.name = 'clickItemCircleSmall_'+index;
        circleMesh.position.set(0,0,0.1);
        thisGroup.add(circleMesh);
        this.clickObjects.push(circleMesh);

        this.circleGeometryList.push(shapeGeometry);
        this.circleMaterialList.push(colorMaterial);
        ...
        if(this.font){
            const textShape = new THREE.BufferGeometry();
            const shapes = this.font.generateShapes( result[index].name, 0.5, 2 );
            const geometryText = new THREE.ShapeGeometry( shapes );
            geometryText.computeBoundingBox();
            const xMid = - 0.5 * ( geometryText.boundingBox.max.x - geometryText.boundingBox.min.x );
            geometryText.translate( xMid, 0, 0 );
            ...
            text.name = 'clickItemTextName_'+index;
            thisGroup.add(text);
            this.clickObjects.push(text);
        ...
        if(this.numFont){
            const textShape = new THREE.BufferGeometry();
            const shapes = this.numFont.generateShapes( result[index].num.toString(), 0.8, 2 );
            const geometryText = new THREE.ShapeGeometry( shapes );
            geometryText.computeBoundingBox();
            const xMid = - 0.5 * ( geometryText.boundingBox.max.x - geometryText.boundingBox.min.x );
            geometryText.translate( xMid, 0, 0 );
            ...
            text.name = 'clickItemTextNum_'+index;
            thisGroup.add(text);
            this.clickObjects.push(text);
        ...
        AnimationAction.time = 100/(result.length-1)*index;
    ...
    scatterCircle(r, init, ring, color, speed) {
        ...
        this.circleGeometryList.push(geometry);
        this.circleMaterialList.push(material);
    ...
    addRectLine(index){
        ...
        this.geometryList.push(geometryRectLine);
        this.materialList.push(materialRectLine);
    ...
    addRing(){
        ...
        this.materialList.push(materialRing);
    }
    ...
    onDocumentMouseDown(event){
        ...
        if(intersects.length > 0) {
            ...
            for(let i = 0;i < intersects.length;i ++){
                if(intersects[i].object.name && intersects[i].object.name.indexOf('clickItem') >= 0){
            ...
            if(clickIcon){
                ...
                console.log(this.result[clickIcon.split('_')[1]]);
                this.interactData(iconClickInteract,this.result[clickIcon.split('_')[1]]);
            }
        }
    }
