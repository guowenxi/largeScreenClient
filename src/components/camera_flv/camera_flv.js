...
export default class CameraFlv extends React.Component {
    constructor(props) {
        ...
        this.cameraSize = {'1x1':[''],'2x2':['','','',''],'3x3':['','','','','','','','','']};
        this.classSize = {'1x1':cssStyle.oneBox,'2x2':cssStyle.twoBox,'3x3':cssStyle.threeBox};
        this.cameraUrl = ['','','',''];
    }
    ...
    callBack(resolve, result) {
        ...
        if (result) {
            ...
            this.cameraUrl[selectedIndex] = result.flv;
                    ...
                    {({opacity}) => {
                        return (
                            <div id={'echarts_map_'+this.props.thisData.id} className={`${cssStyle.box} ${this.classSize[size]}`} style={{opacity,fontSize}} >
                                {this.cameraSize[size] && this.cameraSize[size].map((item,index)=>
                                    <VideoPlayer
