...
import cssStyle from "./name_num_type_two.module.css";

import pieOne from "./images/pie1.svg";
import pieTwo from "./images/pie2.svg";
import pieThree from "./images/pie3.svg";
import {interactData} from "../../common/util";

export default class NameNumTypeTwo extends React.Component {
    constructor(props) {
        super(props);
                ...
                <Motion style={{opacity:spring(this.state.opacity)}}>
                    {({opacity}) =>
                        <div className={cssStyle.box} style={{opacity}}>
                            <img alt="" src={pieThree} className={cssStyle.pie}/>
                            <img alt="" src={pieOne} className={`${cssStyle.pieTwo} ${cssStyle.animateOnTwo}`}/>
                            <img alt="" src={pieTwo} className={`${cssStyle.pie} ${cssStyle.animateOnOne}`}/>
                            <div className={`${cssStyle.topNum} ${cssStyle.flex}`} style={{fontSize:bigNumSize,color:style.bigNumColor}} onClick={this.itemClick.bind(this,0)}>
                                {this.state.resultData[0] != null ? this.state.resultData[0][numKey] :''}
                            </div>
                            <div className={`${cssStyle.topTitle} ${cssStyle.flex}`} style={{fontSize:topTitleSize,color:style.topTitleColor}} onClick={this.itemClick.bind(this,0)}>
                                {this.state.resultData[0] != null ? this.state.resultData[0][titleKey] :''}
                            </div>
                            <div className={cssStyle.itemBox}>
                                <div className={`${cssStyle.num} ${cssStyle.flex}`} style={{fontSize:smallNumSize,color:style.smallNumColor}} onClick={this.itemClick.bind(this,1)}>
                                    {this.state.resultData[1] != null ? this.state.resultData[1][numKey] :''}
                                </div>
                                <div className={`${cssStyle.title} ${cssStyle.flex}`} style={{fontSize:itemTitleSize,color:style.itemTitleColor}}>
