...
import cssStyle from "./address_bar_code.module.css";
import {Motion, spring} from "react-motion";
import {getCompatibleSize, interactData, getQuery} from "../../common/util";
...
export default class Blank extends React.Component {
    ...
    loadData(str) {
      ...
      if (thisData.dataSources.dataUrl) {
        const params = {
          type: 2,
          code: str
        }
        if (thisData.dataSources.dataParams){
          const dataParams = JSON.parse(thisData.dataSources.dataParams)
          axios.get(thisData.dataSources.dataUrl, {params}).then((response) => {
            if (response.data.success) {
              const url = dataParams.url + '/show/'+ dataParams.pageId +'/' + response.data.data +'/'
