import React from "react";
import cssStyle from "../../../common/css/detail.module.css";
import { Scrollbars } from "react-custom-scrollbars";
import { getCloseDom, getCompatibleData, changeThisShow } from "../../../common/detailUtil";
import iconTriangleOne from "../../../common/images/lanjiao_black.svg";
import iconTriangleTwo from "../../../common/images/lanjiao_blue.svg";

export default class BudgetImplementation extends React.Component {
    constructor(props) {
        super(props);
        this.state = {};
        this.getCompatibleData = getCompatibleData.bind(this);
        this.getCloseDom = getCloseDom.bind(this);
        this.changeThisShow = changeThisShow.bind(this);
        this.themeList = ['', cssStyle.themeOne];
        this.themeImgList = [iconTriangleOne, iconTriangleTwo];
    }

    //组件删除时触发函数
    componentWillUnmount() {
    }

    //组件加载触发函数
    componentDidMount() {
    }

    render() {
        const { detail } = this.props;
        const { style } = this.props.thisData;
        const compatibleSize = this.getCompatibleData(style);
        return (
            <div
                className={`${cssStyle.detailBox} ${this.themeList[style.theme]}`}
                style={{ ...this.props.style, backgroundColor: style.bgColor, padding: compatibleSize.padding }}
            >
                <div className={cssStyle.headBox} style={{ height: compatibleSize.titleHeight }}>
                    <div className={cssStyle.head} style={{ fontSize: compatibleSize.titleSize, color: style.titleColor }}>{detail.applyName}差旅经费审批</div>
                </div>
                {this.getCloseDom(style, compatibleSize)}
                <Scrollbars style={{ height: 'calc(100% - ' + compatibleSize.titleHeight + ')', fontSize: compatibleSize.fontSize }}>
                    <div className={cssStyle.itemBox}>
                        <div className={cssStyle.itemHead}>
                            <img alt='' src={this.themeImgList[style.theme]} className={cssStyle.itemHeadIcon} />
                            <div className={cssStyle.itemTitle}>基本信息</div>
                        </div>
                        <table className={cssStyle.itemContent}>
                            <tbody>
                                <tr>
                                    <td className={cssStyle.tdTitle}>申请人</td>
                                    <td className={cssStyle.tdContent}>{detail.applyName}</td>
                                    <td className={cssStyle.tdTitle}>部门</td>
                                    <td className={cssStyle.tdContent}>{detail.depart}</td>
                                </tr>
                                <tr>
                                    <td className={cssStyle.tdTitle}>出差目的</td>
                                    <td className={cssStyle.tdContent}>{detail.travelGoal}</td>
                                    <td className={cssStyle.tdTitle}>交通工具</td>
                                    <td className={cssStyle.tdContent}>{detail.vehicle}</td>
                                </tr>
                                <tr>
                                    <td className={cssStyle.tdTitle}>审批状态</td>
                                    <td className={cssStyle.tdContent}>{detail.status}</td>
                                    <td className={cssStyle.tdTitle}>审批发起时间</td>
                                    <td className={cssStyle.tdContent}>{detail.approvalTime}</td>
                                </tr>
                                <tr>
                                    <td className={cssStyle.tdTitle}>审批处理时间</td>
                                    <td className={cssStyle.tdContent}>{detail.dealTime}</td>
                                    <td className={cssStyle.tdTitle}>审批耗时(分)</td>
                                    <td className={cssStyle.tdContent}>{detail.timeConsume}</td>
                                </tr>
                                <tr>
                                    <td className={cssStyle.tdTitle}>出差行程</td>
                                    <td className={cssStyle.tdContent}>{detail.trip}</td>
                                    <td className={cssStyle.tdTitle}>出差开始日期</td>
                                    <td className={cssStyle.tdContent}>{detail.startDate}</td>
                                </tr>
                                <tr>
                                    <td className={cssStyle.tdTitle}>出差结束日期</td>
                                    <td className={cssStyle.tdContent}>{detail.endDate}</td>
                                    <td className={cssStyle.tdTitle}>同行人</td>
                                    <td className={cssStyle.tdContent}>{detail.colleagues}</td>
                                </tr>
                                <tr>
                                    <td className={cssStyle.tdTitle}>出差类型</td>
                                    <td className={cssStyle.tdContent}>{detail.type}</td>
                                    <td className={cssStyle.tdTitle}>其他</td>
                                    <td className={cssStyle.tdContent}>{detail.other}</td>
                                </tr>
                                <tr>
                                    <td className={cssStyle.tdTitle}>同行</td>
                                    <td className={cssStyle.tdContent}>{detail.peers}</td>
                                    <td className={cssStyle.tdTitle}>同行（协辅警等）</td>
                                    <td className={cssStyle.tdContent}>{detail.peersPeople}</td>
                                </tr>
                                <tr>
                                    <td className={cssStyle.tdTitle}>是否需要午休房</td>
                                    <td className={cssStyle.tdContent}>{detail.needBreak}</td>
                                    <td className={cssStyle.tdTitle}>历史审批人列表</td>
                                    <td className={cssStyle.tdContent}>
                                        {detail.approverList && (
                                        detail.approverList.map((item,index) =>
                                            item ? <span className={cssStyle.fileName} key={index}>{item}</span> :''
                                        )
                                    )}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                </Scrollbars>
            </div>
        );
    }
}