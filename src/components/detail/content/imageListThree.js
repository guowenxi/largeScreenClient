...
import cssStyle from "./imageListThree.module.css";
                ...
                <Scrollbars className='blueScrollbars' ref={this.scrollbars}>
                    <div className={cssStyle.listBox}>
                        {detail && Array.isArray(detail) && detail.map((item,index)=>
                            <div
                                className={cssStyle.imgOneBox} key={index}
                                onClick={() => this.setState({ visible: true, photoIndex: index, showList: detail })}
                            >
                                <img alt={''} src={item} className={cssStyle.img} />
                            </div>
                        )}
                    </div>
                </Scrollbars>
