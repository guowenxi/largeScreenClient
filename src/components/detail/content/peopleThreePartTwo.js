                ...
                {titleList.map((title, index) => {
                        ...
                        if(list && Array.isArray(list)){
                            return list.map((event,eventIndex)=>
                                <React.Fragment key={index+'_'+eventIndex}>
                                    ...
                                    <div className={cssStyle.rowPart} >
                                        <div className={cssStyle.title} >{'事发时间'.split('').map((item,index)=>{return <span key={index}>{item}</span>})}</div>
                                        <div style={{ width: 'calc(100% - 5em)' }} >{event.time}</div>
                                    </div>
                                    <div className={cssStyle.rowPart} >
                                        <div className={cssStyle.title} >{'事件来源'.split('').map((item,index)=>{return <span key={index}>{item}</span>})}</div>
                                        <div style={{ width: 'calc(100% - 5em)' }} >{event.source}</div>
                                    </div>
                                    <div className={cssStyle.rowPart} >
                                        <div className={cssStyle.title} >{'事件类型'.split('').map((item,index)=>{return <span key={index}>{item}</span>})}</div>
                                        <div style={{ width: 'calc(100% - 5em)' }} >{event.type}</div>
                                    </div>
                                    <div className={cssStyle.rowPart} style={{ width: '100%' }} >
                                        <div className={cssStyle.title} >{'事件内容'.split('').map((item,index)=>{return <span key={index}>{item}</span>})}</div>
                                        <div style={{ width: 'calc(100% - 5em)' }} >{event.content}</div>
                                    </div>
                                    <div className={cssStyle.rowPart} style={{ width: '100%' }} >
                                        <div className={cssStyle.title} >{'处置结果'.split('').map((item,index)=>{return <span key={index}>{item}</span>})}</div>
                    ...
                    return (
                        <div className={cssStyle.rowPart} style={{ width: title.width }} key={index}>
                            {title.dataType !== 3 && <div className={cssStyle.title} >{title.name.split('').map((item,index)=>{return <span key={index}>{item}</span>})}</div>}
                            <div className={cssStyle.dataContent} style={title.dataType === 3 ? {width:'100%'}:{}} >{content}</div>
                        </div>
                    )
                })}
            </div>
