...
import cssStyle from "./table_box.module.css";
                    ...
                    {({ opacity }) =>
                        <div className={cssStyle.box} style={{ opacity,fontSize:fontSize}} >
                            <table style={{ width: '100%', height: '100%' }}>
                                <thead>
                                    <tr>
                                        <th style={{width:style.rowWidth}}></th>
                                        {resultData&&resultData.map((item, index) => {
                                            return (
                                                <th key={index} style={{ textAlign: "center" ,color:style.columnColor,fontWeight:style.columnWeight}}>{item.title}</th>
                                            )
                                        })}
                                    </tr>
                                </thead>
                                <tbody>
                                    {style.rowTitleList.map((item, index) => {
                                        return (
                                            <tr key={index}>
                                                <td style={{color:style.rowColor,fontWeight:style.rowWeight}}>{item.title}</td>
                                                {resultData&&resultData.map((rowItem, rowIndex) => {
                                                    return(
                                                        <td key={rowIndex} style={{ textAlign: "center",fontSize:style.itemFontSize,color:style.itemColor,fontWeight:style.itemFontWeight}}>
                                                            {rowItem.data[index]}
                                                            <img alt="" src={images[index]} className={cssStyle.imageStyle}/>
                                                        </td>
                                                    )
                                                })}
                                            </tr>
