            ...
            <div style={this.props.style} className={this.props.className} ref={this.boxRef}>
                {width && height ? (
                    <svg x="0" y="0" viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: '100%' }}>
                        <linearGradient id={`SVGID_2_${this.id}`} gradientUnits="userSpaceOnUse" x1={`${width-11.063}`} y1={`${height-30.9372}`} x2={`${width-52.9372}`} y2={`${height+10.937}`}>
                            <stop offset="0" style={{stopColor:"#FFFFFF",stopOpacity:"0"}} />
                            <stop offset="0.9807" style={{stopColor:"#FFFFFF",stopOpacity:"0.9807"}} />
                            <stop offset="1" style={{stopColor:"#FFFFFF"}} />
                        </linearGradient>
                        <path d={`  M17.2,0.5 h${width-18.4} l-16,${height-1} H1.2 L17.2,0.5z`} style={{fillRule:"evenodd",clipRule:"evenodd",fill:"#2579F2",fillOpacity:"0.2",stroke:`url(#SVGID_2_${this.id})`,strokeMiterlimit:"10",strokeOpacity:"0.6"}} />
                    </svg>
                ) : ''}
            </div>
        );
